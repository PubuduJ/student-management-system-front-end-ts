...
import {StudentType, StudentErrorMsgType} from "../types/types";
...
function SaveStudent() {
    const [saveErrMessage, setSaveErrMessage] = useState<string>("");
    ...
    // When save button is pressed, student will be saved to the database.
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        if (!output.nic || !output.name || !output.address || !output.contact) {
            setSaveErrMessage("Inputs didn't check out");
            return;
        }
        try {
            await postCall(output);
            setResponseMessage("Student successfully saved to the database");
        } catch (err: any) {
            if (err.response) {
        ...
        <div className={"centered-element"}>
            ...
            <div className="student-container">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <h4 className={"error-text"}>{saveErrMessage}</h4>
